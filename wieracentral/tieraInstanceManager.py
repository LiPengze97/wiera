...
from WieraTieraInstanceIface import *
from WieraTieraInstanceIface.ttypes import *
...
from TieraInstanceWieraIface import *
from TieraInstanceWieraIface.ttypes import *

class WieraTieraInstanceHandler:
	def __init__(self, policy, tiera_instance_manager):#iera_instance_manager):
		self.policy = policy
		self.tiera_instance_manager = tiera_instance_manager
		self.lock = threading.Lock()

	def updateMonitoringData(self, monitoring_info):
		json_data = json.loads(monitoring_info)
		result = self.tiera_instance_manager.update_monitoring_info(json_data)
...
#			monitoring_info = self.tiera_instance_manager.get_monitoring_info(['ebs-st1', 'ebs-gp2', 's3', 'standard-disk', 'premium-p10'])
			monitoring_info = self.tiera_instance_manager.get_monitoring_info(['ebs-st1', 'ebs-gp2', 's3'])
#<F12			pprint(monitoring_info)
			...
			for hostname in self.tiera_instance_manager.instance_list:
			...
			for instance in self.tiera_instance_manager.instance_list:
				client = self.tiera_instance_manager.get_tiera_instance_client(instance)
	...
	def registerTieraInstance(self, instance_info, instance_ip):
...
#			print 'lock acquired'
			...
			if instance_info != None:
				...
				stored_server_ip = self.tiera_instance_manager.policy.find_ip_by_hostname(hostname)

				if stored_server_ip == None:
					reason = 'Connection from ' + hostname + ':' + ip + ' is not in the expected location of Tiera server thus rejected.'
					print reason
					result['result'] = False
					result['value'] = reason
					...
					self.tiera_instance_manager.add_tiera_instance(hostname, ip, application_port, peer_port, instance_port)
					...
					if self.policy.check_instance_cnt() == True:
						peer_info = {}
						peer_info['value'] = self.tiera_instance_manager.get_connected_instances()
					...
					print '[TIM-' + self.tiera_instance_manager.policy.policy_id + ']' + hostname + '(' + ip + ':' + str(instance_port) + ') is registered.'
			else:
				result['result'] = False
...
#			print 'lock released'
	...
	def requestPolicyChange(self, policy):
		...
		response = {}
		failed_list = self.tiera_instance_manager.broadcast(policy, 'policyChange')
...
class TieraInstanceManager:
	...
	def remove_tiera_instance(self, hostname):
#		self.lock.acquire()
		if hostname in self.instance_list:
			del self.instance_list[hostname]
#		self.lock.release()

	def get_tiera_instance_client(self, hostname):
		if hostname in self.instance_list:
...
#		print instance_list
	...
	def add_tiera_instance(self, hostname, ip, application_port, peer_port, instance_port):
...
#		print self.instance_list[hostname]
		...
		client = TieraInstanceWieraIface.Client(protocol)
		...
		tiera_server_cnt = len(self.policy.get_available_host_list())
		connected_cnt = len(self.instance_list)

		if connected_cnt == tiera_server_cnt:
	...
	def run_forever(self):
		print '[TIM] Tiera Instance Manager is ready for Tiera Instance port: ' + str(self.port) 
		self.server.serve()
