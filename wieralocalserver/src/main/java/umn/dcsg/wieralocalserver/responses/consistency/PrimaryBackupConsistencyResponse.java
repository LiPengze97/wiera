 ...
 */
public class PrimaryBackupConsistencyResponse extends Response {
    ...
    public boolean respond(Map<String, Object> responseParams) {
        ...
        if (LocalServer.getHostName().equals(strPrimaryHostname) == true) { //Check primary
            if (m_strRelatedEventType.equals(ACTION_PUT_EVENT) == true) {
                responseList.add(StoreResponse.class);
                ...
                if (responseParams.containsKey(LAZY_UPDATE) == true && (boolean) responseParams.get(LAZY_UPDATE) == true) {
                    responseList.add(QueueResponse.class);
                } else {
                    responseList.add(BroadcastResponse.class);
                }
	...
	public Object getInternal(String strKey, OperationLatency latencyInfo)
	{
		...
		try
		{
			...
			PeerInstanceIface.Client client = m_peerInstanceManager.getPeerClient(strPeer);
			...
			{
				System.out.println("Read local data. LocalInstanceClient is null peer : " + strPeer);
